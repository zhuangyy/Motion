...
namespace ZForge.Configuration
{
	...
	/// </summary>
	public class XMLConfigSetting : IConfigSetting
	{
		/// <summary>
		...
		/// </param>
		public XMLConfigSetting(XmlNode node)
		{
			if (node == null)
		...
		{
			//#warning Code in ChildCount(bool) is NOT optimised. If you can help me out with a better algorithm selecting unique child names (probably using XPath), please contact me at axos88@gmail.com Thanks!
			IList<string> names = GetChildrenNames(unique);
			if (names != null)
				return names.Count;
			else
				return 0;
		...
		/// </summary>
		/// <remarks>Read only property</remarks>
		public IList<XMLConfigSetting> Children()
		{
			if (GetChildCount(false) == 0)
				return null;
			List<XMLConfigSetting> list = new List<XMLConfigSetting>();

			foreach (XmlNode achild in node.ChildNodes)
			{
				list.Add(new XMLConfigSetting(achild));
			}
			return list;
		}
		/// <summary>
		...
		/// </returns>
		public IList<XMLConfigSetting> GetNamedChildren(String name)
		{
			...
			List<XMLConfigSetting> css = new List<XMLConfigSetting>();
			foreach (XmlNode achild in xmlnl)
			{
				css.Add(new XMLConfigSetting(achild));
			}
			return css;
		}
		...
		public IConfigSetting this[string path]
		{
			get
			{
				...
				return new XMLConfigSetting(selectednode);
			}
		}
		...
		public bool Validate()
		{
			...
			{
				foreach (XMLConfigSetting cs in this.Children())
				{
					if (!cs.Validate())
						return false;
				}
		...
		public void Clean()
		{
			...
			if ((GetChildCount(false) == 0) && (this.Value == ""))
				this.Remove();
		}
		...
		/// </summary>
		/// <param name="x"></param>
		public void Copy(XMLConfigSetting x)
		{
			this.RemoveChildren();
			foreach (XmlNode n in x.Node.ChildNodes)
			{
				XmlNode v = node.OwnerDocument.ImportNode(n, true);
				node.AppendChild(v);
			}
		}
	}
